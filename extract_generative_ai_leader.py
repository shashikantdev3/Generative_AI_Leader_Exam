#!/usr/bin/env python3
"""Extract Generative AI Leader exam questions and generate HTML page."""
...
def get_generative_ai_leader_questions():
    """Fetch all Generative AI Leader questions with associated data."""
    conn = psycopg2.connect(**DB_CONFIG)
    cur = conn.cursor()

    # Get Generative AI Leader exam ID
    cur.execute("""
        SELECT exam_id, exam_code, exam_name
        FROM db_exams
        WHERE exam_code = 'generative-ai-leader'
        LIMIT 1
    """)

    exam_result = cur.fetchone()
    if not exam_result:
        print("Generative AI Leader exam not found in database!")
        return [], None, None

    exam_id, exam_code, exam_name = exam_result
    print(f"Found exam: {exam_code} - {exam_name} (ID: {exam_id})")

    # Get all questions for Generative AI Leader
    cur.execute("""
        SELECT
            q.question_id,
            q.discussion_id,
    ...
    print(f"Found {len(questions)} questions for Generative AI Leader")
...
def copy_images(questions_data, base_folder):
    ...
    for q in questions_data:
        ...
        question_folder = os.path.join(base_folder, 'generative-ai-leader', str(discussion_id))
...
<body>
    <div class="container">
        <h1>{escape_html(exam_name)} - All Questions and Answers <span class="provider-badge">Google Cloud</span></h1>
        <div class="exam-info">
            <strong>Exam Code:</strong> {escape_html(exam_code)}<br>
            <strong>Exam Name:</strong> {escape_html(exam_name)}<br>
            <strong>Provider:</strong> Google Cloud<br>
        ...
        if q['images']:
            ...
            for img in q['images']:
                ...
                if discussion_id and file_name:
                    ...
                    rel_path = f"generative-ai-leader/{discussion_id}/{file_name}"
...
def main():
    print("="*80)
    print("Google Cloud Generative AI Leader Exam - Data Extraction Tool")
    print("="*80)

    print("\nExtracting Generative AI Leader questions from database...")
    questions_data, exam_code, exam_name = get_generative_ai_leader_questions()
    ...
    base_folder = 'Generative_AI_Leader_Exam'
